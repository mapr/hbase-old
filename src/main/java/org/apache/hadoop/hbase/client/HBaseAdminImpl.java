...
//  private final HConnection connection;
 ...
 /**
   ...
   * @throws ZooKeeperConnectionException if unable to connect to zookeeper
   */
  public HBaseAdminImpl(HConnection connection)
      throws MasterNotRunningException, ZooKeeperConnectionException {
  ...
  public HTableDescriptor[] listTables(String regex) throws IOException {
    regex = FSUtils.adjustTableNameString(regex);
  ...
  throws IOException {
    byte[] table_name = FSUtils.adjustTableName(desc.getName());
  ...
  public void deleteTable(final byte [] tableName) throws IOException {
    isMasterRunning();
    byte[] table_name = FSUtils.adjustTableName(tableName);
    HTableDescriptor.isLegalTableName(table_name);
  ...
  throws IOException, InterruptedException {
    CatalogTracker ct = getCatalogTracker();
    byte[] table_region_name = FSUtils.adjustTableName(tableNameOrRegionName);
    try {
      Pair<HRegionInfo, ServerName> regionServerPair
        = getRegion(table_region_name, ct);
   ...
   * @throws IOException
   */
  public Pair<HRegionInfo, ServerName> getRegion(final byte[] tableNameOrRegionName,
