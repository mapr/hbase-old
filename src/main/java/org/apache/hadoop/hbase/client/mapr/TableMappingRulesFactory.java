    ...
    try {
      ...
      try {
        return ruleFactory_.getImplementorInstance(
          conf, conf.get("hbase.mappingrule.impl", "com.mapr.fs.TableMappingRules"),
          new Object[] {conf}, new Class[] {Configuration.class});
      } catch (Throwable t) {
        if (t.getCause() != null && t.getCause() instanceof ClassNotFoundException) {
          LOG.info("Could not find TableMappingRules class, assuming HBase only cluster.");
