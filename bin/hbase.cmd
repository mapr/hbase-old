...
if exist "%HADOOP_HOME%\bin\%HADOOP_IN_PATH%" (
  ...
  set OTHER_CLASSPATH=
  for /f %%i in ('dir /b %HADOOP_HOME%\lib\hadoop*.jar') do set OTHER_CLASSPATH=!OTHER_CLASSPATH!;%HADOOP_HOME%\lib\%%i
  for /f %%i in ('dir /b %HADOOP_HOME%\lib\mapr*.jar') do set OTHER_CLASSPATH=!OTHER_CLASSPATH!;%HADOOP_HOME%\lib\%%i
  for /f %%i in ('dir /b %HADOOP_HOME%\lib\libprotodefs*.jar') do set OTHER_CLASSPATH=!OTHER_CLASSPATH!;%HADOOP_HOME%\lib\%%i
  set CLASSPATH=!CLASSPATH!;!OTHER_CLASSPATH!
)
...
@rem set corecommands=shell master regionserver thrift thrift2 rest avro hlog hbck hfile zookeeper zkcli
set corecommands=shell hlog hbck hfile
...
@rem  echo   zkcli            run the ZooKeeper shell
@rem  echo. 
@rem  echo PROCESS MANAGEMENT
@rem  echo   master           run an HBase HMaster node 
@rem  echo   regionserver     run an HBase HRegionServer node 
@rem  echo   zookeeper        run a Zookeeper server
@rem  echo   rest             run an HBase REST server 
@rem  echo   thrift           run the HBase Thrift server 
@rem  echo   thrift2          run the HBase Thrift2 server 
@rem  echo   avro             run an HBase Avro server 
  echo. 
